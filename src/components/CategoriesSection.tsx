...
export const CategoriesSection: React.FC = () => {
  ...
  const { result: { data: categoryData }, query: { isLoading: categoryLoading } } = useList({
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap gap-4 md:gap-6 items-center justify-center">
          {categoryData?.map((category) => {
            const IconMapped = (LucideIcons as any)[category.icon]
            return (
              <div className="sm:w-1/2 md:w-1/4 lg:w-1/7 flex-1 min-w-[160px] max-w-[190px] sm:min-w-[140px] md:min-w-[160px] cursor-pointer" key={category?.category_id}>
                <div
                  key={category?.category_id}
                  onClick={() => push('/courses', { state: { category: category?.name, title: `Courses in ${category?.name}`, viewall: true } })}
